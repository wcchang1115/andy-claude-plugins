...
- **Out of scope: state it only when this PR leaves something out on purpose, as a design decision.** Do not mention a problem that you found while exploring the code. If that problem needs a fix, propose a new tracker issue to the user. Do not mention that issue in the PR, unless the two PRs ship as a bundle.
- **No session history.** Do not mention an earlier wrong draft, a mistake fixed during the session, or what caused a similar problem before.
